...
$("a.intro__nav-link").on("click", function (e) {
  e.preventDefault();
...
$(function () {
  ...
  $('.header__burger').click(function () {
    ...
    $('.menu__nav').toggleClass('open-menu');
  });
...
restart_quiz.onclick = () => {
    ...
    timeText.textContent = "Времени нет!"; //change the text of timeText to Time Left
...
next_btn.onclick = ()=>{
    if(que_count < questions.length - 1){ //if question count is less than total question length
        ...
        startTimer(timeValue); //calling startTimer function
        startTimerLine(widthValue); //calling startTimerLine function
        timeText.textContent = "Время есть"; //change the timeText to Time Left
        next_btn.classList.remove("show"); //hide the next button
    }else{
        clearInterval(counter); //clear counter
        clearInterval(counterLine); //clear counterLine
...
function showResult(){
    ...
    if (userScore > 3){ // if user scored more than 3
        ...
        let scoreTag = '<span>🎉, Вы смогли набрать <p>'+ userScore +'</p> из <p>'+ questions.length +'</p>очков</span>';
...
function startTimer(time){
    ...
    function timer(){
        ...
        if(time < 0){ //if timer is less than 0
            clearInterval(counter); //clear counter
            timeText.textContent = "Времени нет"; //change the time text to time off
            const allOptions = option_list.children.length; //getting all option items
            let correctAns = questions[que_count].answer; //getting correct answer from array
            for(i=0; i < allOptions; i++){
                if(option_list.children[i].textContent == correctAns){ //if there is an option which is matched to an array answer
...
function queCounter(index){
    ...
    let totalQueCountTag = '<span><p>'+ index +'</p> из <p>'+ questions.length +'</p> вопросов</span>';
